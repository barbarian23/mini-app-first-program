...
import { wxTemp } from "./utils/wxTemp";
...
export default {
  ...
  async onShow(options) {
    let system = wxTemp.getAccountInfoSync();
    ...
    if (options.query.goType == 1) {
      ...
      if (options.query.dynamicId)
        url +=
          "?dynamicId=" +
          options.query.dynamicId +
          "&companyId=" +
          options.query.companyId +
          "&cardId=" +
      ...
      if (
        ...
        options.query.shareId !== this.preShareId &&
        this.isLogin
      ) {
