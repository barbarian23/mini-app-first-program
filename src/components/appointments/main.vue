            ...
            <scroll-view scroll-x
              ...
              <div class="">
                <div class="nav-item"
                     ...
                     @click="menu_tap(navItem.productsTypeId)">{{navItem.productsTypeName}}
                </div>
              </div>
            </scroll-view>
...
export default {
  name: '',
  components: { RightFloat, NoData, getPhoneNumberGoods, SelfSwiper, AuthenticationTag, FloatButtons, NavBarByUser },
  data() {
    return {
      lang: LangUtils.getLangSrc().components.appointments,
  ...
  onShareAppMessage(e) {
    ...
    addShareRecord(this.COMPANYID, 3, '', uuid).then(res => { }, err => { });
  ...
  methods: {
    ...
    toSearchGoods() {
      wx.navigateTo({ url: '/pages/appointmentPack/searchGoods/main?companyId=' + this.COMPANYID });
    },
    clickRightRowEvent() {
    ...
    getTypeMenus() {
      ...
      }, '', '/products/selectProductsTypeList/V2').then(res => {
        if (res) {
          this.menu = [{ productsTypeName: '全部', productsTypeId: -10 }, ...res];
        } else {
    ...
    getProds() {//获取产品
      ...
      }, '', '/products/selectProductsPage/V2').then((data) => {
        wx.hideLoading();
        if (data) {
          // wx.showToast({
          //     title: '更新成功',
