<template>
  <div class="bgfff"
       :class="iscart || isorder ? 'bradius10' : '' ">
    <div class="pl15 lh44 posre disflex jsbet pr21 "
      ...
      <div class="disflex align-cen">
        ...
        <img src="https://you-one-stand.oss-cn-shanghai.aliyuncs.com/yimai_photos/user/company_name.png"
             alt=""
             class="w16 h16 mr8">
      ...
      <span v-if="order_type==1"
            class="corange fs14">{{ lang.text_0 }}</span>
      <span v-if="orderData.assembleId? (orderData.assembleSuccess && order_type == 2): order_type == 2"
            class="corange fs14">{{ lang.text_1 }}</span>
      <span v-if="!orderData.assembleSuccess && order_type==2 && orderData.assembleId"
            class="corange fs14">{{ lang.text_2 }}</span>
      <span v-if="order_type==3"
            class="corange fs14">{{ lang.text_3 }}</span>
      <span v-if="order_type==4"
            class="cblue fs14">{{ lang.text_4 }}</span>
      <span v-if="order_type==5"
            class="corange fs14">{{ lang.text_5 }}</span>
      <span v-if="order_type==6"
            class="corange fs14">{{ lang.text_6 }}</span>
      <span v-if="order_type==7"
            class="corange fs14">{{ lang.text_7 }}</span>
    </div>
    <div class="disflex posre pb21 flex-c-c order-content"
      ...
      <!--只有等待付款是需要显示秒杀倒计时-->
      <SeckillCuntdown class=" pt15 pl15"
                       v-if="!iscart && prod_item.isKill && order_type==1"
                       :diffTime="parseInt(prod_item.killEndTime/1000)" />
      <SeckillCuntdown class=" pt15 pl15"
                       v-else-if="iscart && prod_item.isKill && order_type==1"
                       :diffTime="parseInt(prod_item.killEndTime/1000)" />
      <SeckillCuntdown class=" pt15 pl15"
                       v-else-if="order_type==null && prod_item.isKill && order_type==1"
                       :diffTime="parseInt(prod_item.killEndTime/1000)" />
      <div :class="iscart?'pl45 pt20':'pl15 pt15'">
        ...
        <div class="disflex w100p pr16">
          <img :src="prod_item.photoUrl"
               alt=""
               class="w70 h70 mr11 bradius5"
               mode="aspectFill">
          <div class="flex1">
            ...
            <div class="disflex mt5"
              ...
              <span class="be8 pl7 pr8 lh20 fs12 ca8 mr8 bradius3">
                {{prod_item.specName ? prod_item.specName : ''}}{{prod_item.specAttribute}}
              </span>
            </div>
            <div class="mt8"
                 v-else>
              <span class="pl7 pr8 lh20 fs12 ca8 mr8">
                {{prod_item.specName ? prod_item.specName : ''}}x {{prod_item.num}}
              </span>
              <div>
                <span class="pl7 pr8 lh20 fs12 ca8 mr8">￥{{prod_item.isKill ? prod_item.killPrice : prod_item.specPrice}}</span>
              </div>
            </div>
            <!--加减-->
            <div class="disflex jsbet align-cen"
                 v-if="iscart"
                 @click.stop="''">
              <span class="cdeeporange fs14"><span>￥</span><span>{{prod_item.isKill ? prod_item.killPrice : prod_item.specPrice}}</span></span>
              <div class="counter disflex number-change-btn-group">
                <span class="textc be8 lh25 w25 minus ca8"
                      @click.stop="prodChangeNum(index1,k,-1)">
                  <img src="https://you-one-stand.oss-cn-shanghai.aliyuncs.com/product-index/20190505113629.png"
                       class="w10 h10" />
                </span>
                <span class="bte8 bbe8 fs12 c68 w40 lh25 textc">{{prod_item.num}}</span>
                <span class="textc be8 lh25 w25 add ca8"
                      @click.stop="prodChangeNum(index1,k,1)">
                  <img src="https://you-one-stand.oss-cn-shanghai.aliyuncs.com/product-index/20190505113647.png"
                       class="w10 h10" />
                </span>
              </div>
            </div>
            <!--价格-->
            <div class="textr fs14 c38"
                 v-if="isorder">
              ￥{{prod_item.allPrice}}
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
      lang: LangUtils.getLangSrc().components.orderItems,
      company_check: false
    };
  },
  mounted() {
    console.log(!this.orderData.assembleSuccess);
  },
  methods: {
    ...
    toCompany() {
      ...
      wx.switchTab({ url: "../Products/main" });
    }
  },
  watch: {}
};
