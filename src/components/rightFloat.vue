  ...
  >
    <FormId>
      <div class="pt16" @click="page_turn('shopCart')">
        <img
          src="https://you-one-stand.oss-cn-shanghai.aliyuncs.com/yimai_photos/user/cart_gray.png"
          alt
          class="w24 h24"
        />
        <p class="pt5">{{ lang.text_0 }}</p>
      </div>
    </FormId>
     <FormId v-if="!isSelf">
      <GetPhoneNumber @next="toIMPage">
        <div class="pt16 posre">
          <img
            src="https://you-one-stand.oss-cn-shanghai.aliyuncs.com/yimai_photos/user/msg_gray.png"
            alt
            class="w24 h24"
          />
          <span class="right_point" v-if="currentNum>0">{{currentNum}}</span>
          <p class="pt5">{{ lang.text_1 }}</p>
        </div>
      </GetPhoneNumber>
    </FormId>
<!--    <FormId>-->
...
<!--        <img-->
<!--          src="https://you-one-stand.oss-cn-shanghai.aliyuncs.com/yimai_photos/user/user_gray.png"-->
<!--          alt-->
<!--          class="w24 h24"-->
<!--        />-->
<!--        <p class="pt5">{{ lang.text_2 }}</p>-->
<!--      </div>-->
<!--    </FormId>-->
    <div class="pt16" @click="clickRightRowEvent">
      <img
        src="https://you-one-stand.oss-cn-shanghai.aliyuncs.com/yimai_photos/user/right_row.png"
        alt
...
import LangUtils from '../utils/LangUtils';
  ...
  export default {
    ...
    data(){
      return{
        lang: LangUtils.getLangSrc().components.rightFloat,
        timer: null,
      }
    },
    mounted(){
    ...
    methods:{
      ...
      getCompany(){
        ...
        setTimeout(() => {
          WXAJAX.POST({
            cardId: cardId || 1,
            refresh:  "",
          }, "", "/businessCard/cardDetails").then((data, code) => {
            if (data) {
      ...
      clickRightRowEvent() {
        ...
        toIMPage(){
            ...
            WXAJAX.ToIMPage(this.currentCompany,1, this.specId);
        },
      page_turn(url){
        wx.navigateTo({url:'/pages/'+url+'/main?type='+this.intoType
