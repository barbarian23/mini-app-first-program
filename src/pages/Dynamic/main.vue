<template>
  <div class="posre borderbox dynamic"
    ...
    <scroll-view :style="{ height: scrollContentHeight + 'px' }"
      ...
      <div style="position: relative; z-index: 2;"
        ...
        <!-- <div class="fs18 cfff disflex pl15 pr16 row-reverse dynamic-title">
          <img :src="company_msg.admin_logo"
                @click="authSubscribeComment"
               alt=""
        ...
        <template v-if="dynamic_lists.length">
          <div v-for="(dynamic_item, k1) in dynamic_lists"
               :key="k1"
    ...
    <div class="fix_bottom bgfff edit_input trans2"
      ...
      <div class="bgfff pt3 pb5 pl10 disflex jsbet pr11 borderbox align-cen">
        ...
        <span class="bgblue cfff w50 textc lh30 fs16 bradius5 disblock ml10"
              @click.stop="inputConfirm">{{ lang.text_1 }}</span>
      </div>
    </div>
    <FloatButtons class="float-buttons"
                  ...
                  @talk.stop="talk"
                  @loginGuide.stop="loginGuide"></FloatButtons>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      lang: LangUtils.getLangSrc().Dynamic,
  ...
  methods: {
    ...
    authSubscribeComment(){
      authSubscribeComment(this.subscriptionNew);
    },
    //下拉刷新
    scrolltoupper(e) {
      wx.showLoading({
    ...
    getAd() {
      const result = this.getPlateAds(3);
      if(result){
        result.then(res => {
          this.banners = res
    ...
    inputConfirm() {
      ...
      WXAJAX.POST(
        ...
        if (
          ...
          this.dynamic_lists[this.add_comments_index].commentNum = this.dynamic_lists[this.add_comments_index].commentModelList.length
          this.$set(
            this,
            "dynamic_lists",
            JSON.parse(JSON.stringify(this.dynamic_lists))
  ...
  onShareAppMessage(e) {
    ...
    addShareRecord(this.COMPANYID, 4, dynamicid, uuid).then(res => { }, err => { });
