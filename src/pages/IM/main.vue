<template>
  <div class="h100p toppage pb50" :class="pageFixed ? 'fix' : '' " id="CONT" @click="phrases = isMore = false">
    <!--顶部-->
    <div class="pl35 pt10 textc pr35 pb9 bgfff disflex jsbet fix_top borderbox fs12 c78">
      <div @click="addWx">
        <img src="https://you-one-stand.oss-cn-shanghai.aliyuncs.com/yimai_photos/user/im_msg.png" alt="" class="w30 h30">
        <p class="pt8">{{ lang.text_0 }}</p>
      </div>
      <div @click="makePhone">
        <img src="https://you-one-stand.oss-cn-shanghai.aliyuncs.com/yimai_photos/user/tel_blue.png" alt="" class="w30 h30">
        <p class="pt8">{{ lang.text_1 }}</p>
      </div>
      <label v-if="!myPhone">
        <img src="https://you-one-stand.oss-cn-shanghai.aliyuncs.com/yimai_photos/user/im_link.png" alt="" class="w30 h30">
        <p class="pt8">{{ lang.text_2 }}</p>
      ...
      <div class="im_by_time" v-if="msgCont" v-for="(msgitem,k) in msgCont.data" :key="k">
        ...
        <template v-else>
          ...
          <div class="im_right disflex pl22 pr15 im_item flex-end" v-if="msgitem.sendId== myid">
            <!--消息状态-->
            <div class="disflex column-reverse w33 fs12 ca8" v-if="msgitem.type==0">{{ lang.text_3 }}</div>
            <div class="disflex column-reverse w33 fs12 ca8" v-if="msgitem.type==1">{{ lang.text_4 }}</div>
            <!--语音-->
              ...
              <div class="disinblock">
                <div class='output-audio-right'>
                  <div class='audio-right' @click="audioPlay(msgitem.message, k)">
                    <img class='ico-right' v-if="urls !== msgitem.message"  src='https://you-one-stand.oss-cn-shanghai.aliyuncs.com/one-stand/card/yuyin-ico.png'/>
                    <img class='ico-right' v-else src='https://you-one-stand.oss-cn-shanghai.aliyuncs.com/one-stand/card/yuyin-gif.gif'/>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div v-else-if="msgitem.contentType && msgitem.contentType === 2">
              <img mode="aspectFill"  :src="msgitem.message" @click="previewImg(msgitem.message)"  style="border-radius: 10rpx;max-height: 310rpx;max-width: 310rpx"/>
            </div>
            <!--视频-->
            <div v-else-if="msgitem.contentType && msgitem.contentType === 4">
              <video show-play-btn="false" enable-progress-gesture="false" vslide-gesture-in-fullscreen="false" play-btn-position="center"  style="border-radius: 10rpx;max-height: 310rpx;max-width: 310rpx" controls="false"  :src="msgitem.message" ></video>
            </div>
            <!--信息-->
            <div v-else>
              ...
              <!-- <div class="im_right_msg  pl15 pt14 pr18 bgfff bradius5 w250 be8 posre" v-if="msgitem.messageType==2">
                ...
                <p class="fs12 ca8 pt10 pb9">{{ lang.text_5 }}</p>
                <div class="disflex jsbet fs12 c38 lh30 textc wrap">
                  <span class="be8 bgf5f6 borderbox w104 mb10 " @click="page_turn('Products')">{{ lang.text_6 }}</span>
                  <span class="be8 bgf5f6 borderbox w104 mb10" @click="page_turn('index')">{{ lang.text_7 }}</span>
                  <span class="be8 bgf5f6 borderbox w104 mb10 " @click="page_turn('WebSite')">{{ lang.text_8 }}</span>
                  <span class="be8 bgf5f6 borderbox w104 mb10" @click="page_turn('Dynamic')">{{ lang.text_9 }}</span>
                </div>
                <span class="triangle_border_right"></span>
              </div> -->
            </div>
          ...
          <div class="im_left disflex pl15 pr22 im_item" v-if="msgitem.sendId==sendId">
            <!--头像-->
            <img :src="userlogo || 'https://you-one-stand.oss-cn-shanghai.aliyuncs.com/yimai_photos/crm/card1_user.png'" alt="" mode="aspectFill" class="im_left_img mr10 w40 h40 bradius5">
            <!--语音-->
            <div v-if="msgitem.contentType && msgitem.contentType === 3">
              <div class="disinblock">
                <div class='output-audio-left'>
                  <div class='audio-left' @click="audioPlay(msgitem.message, k)">
                    <img class='ico-left' v-if="urls !== msgitem.message"  src='https://you-one-stand.oss-cn-shanghai.aliyuncs.com/one-stand/card/yuyin-ico.png'/>
                    <img class='ico-left' v-else src='https://you-one-stand.oss-cn-shanghai.aliyuncs.com/one-stand/card/yuyin-gif.gif'/>
                  </div>
                </div>
              </div>
            </div>
            ...
            <!--视频-->
            <div v-else-if="msgitem.contentType && msgitem.contentType === 4">
              <video style="border-radius: 10rpx;max-height: 310rpx;max-width: 310rpx" controls="false"  :src="msgitem.message" ></video>
            </div>
            <!--信息-->
            <div v-else>
            ...
            <!--信息状态-->
            <div class="disflex textr column-reverse w33 fs12 ca8" v-if="msgitem.type==0">{{ lang.text_3 }}</div>
            <div class="disflex textr column-reverse w33 fs12 ca8" v-if="msgitem.type==1">{{ lang.text_4 }}</div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 联系我
      contactMe(){
        console.log(this.$root.$mp.query.userId)
        WebSocket.sendMessage({
          code: 103,
      ...
      getCommonPhrases(){
        WXAJAX.POST({
          ...
          type: 1,
        }, '', '/personal/selectUserCommonList').then(res=>{
      ...
      getTime(){
        ...
        if(this.msgCont.data){
          let times = this.msgCont.data.map((res, idx) => {
            if ( res.messageType == 3 ) priceObj[idx] = res.message.price ? res.message.price /100 : 0;
            return res.time
          });
          let asd = {};
      ...
      dateF(data){
        ...
        if (year === year1 && month === month1 && day === day1) {
          ...
          result =  year1 + "年" +  month + "月" + day + '日' + h + ":" + m;
        }
        return result
      },
//            发送消息
...
.im_by_time .im_item {
  ...
  .triangle_border_right, .triangle_border_left,.triangle_border_right1 {
    position: absolute;
    top: 24upx;
    display: block;
    width: 24upx;
  ...
  .triangle_border_right,.triangle_border_right1 {
    right: -12upx;
    transform: rotate(-45deg);
  }
  ...
  .noread .triangle_border_left, .noread .triangle_border_right,.noread .triangle_border_right1 {
    background: #51CDCB;
    border: none;
  }
</style>
