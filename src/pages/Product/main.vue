    ...
    <scroll-view :style="{height: scrollContentHeight+'px'}"
      ...
      <div class="bgfff">
        <div class="fs18 cfff disflex pl15 pr16 company-title">
          <img :src="company_msg.company_logo || 'https://you-one-stand.oss-cn-shanghai.aliyuncs.com/yimai_photos/user/card1_user.png'"
               mode="aspectFill"
               alt=""
...
<!--              <div class="voucher-center-btn"-->
<!--                   @click="showDevelopingTips">-->
<!--                领劵中心-->
              <!-- showDevelopingTips -->
              <div class="voucher-center-btn 111111" @click="couponCenter">
                  领劵中心
              </div>
              <div class="disflex align-cen">

                <button class='w24 pl0 pr0 bgfff contentbox textr disflex align-cen mr23'
                        open-type="share"
                        hover-class="other-button-hover">
                  <img src="https://you-one-stand.oss-cn-shanghai.aliyuncs.com/yimai_photos/user/share2.png"
                       alt=""
                       class="w20 h20 mr7"
                       style="flex: 0 0 40upx">
                </button>
                <img src="https://you-one-stand.oss-cn-shanghai.aliyuncs.com/product-index/20190418161144.png"
                     alt=""
                     class="w20 h20"
                     @click="collect"
                     v-if="isCollect">
                <img src="https://you-one-stand.oss-cn-shanghai.aliyuncs.com/yimai_photos/user/star.png"
                     alt=""
                     class="w20 h20 posre"
                     @click="collect"
                     v-if="!isCollect">
      ...
      <div>
        <div class="nav_top">
          <div class="posre pl15 pr42 bgfff">
            ...
            <img src="https://you-one-stand.oss-cn-shanghai.aliyuncs.com/yimai_photos/user/search.png"
                 alt=""
                 @click="toSearchGoods"
    ...
    <!-- 商城优惠券 -->
    <shopCoupon  ref="shopCoupon" @></shopCoupon>
  </div>
</template>
...
export default {
  name: '',
  components: { RightFloat, NoData, getPhoneNumberGoods, SelfSwiper,
  AuthenticationTag, FloatButtons, NavBarByUser, LoginIntercept,shopCoupon },
  data() {
    return {
      lang: LangUtils.getLangSrc().Product,
      ...
      nav_top: false,
      banners: ['http://you-one-stand.oss-cn-shanghai.aliyuncs.com/product-index/20190418160834.png'],
      isShow: true,
  ...
  onShareAppMessage(e) {
    ...
    addShareRecord(COMPANYID, 2, '', uuid).then(res => { }, err => { });
    return {
      path: '/pages/Products/main?cardId=' + CARDID + '&companyId=' + COMPANYID + '&goType=1&shareId=' + uuid,
      success(e) {
        wx.showShareMenu({
  ...
  methods: {
    ...
    talk() {
      ...
      wx.navigateTo({        url: '../IM/main?userId=' + this.currentCompany.userId
    ...
    getAd() {
      this.getPlateAds(1).then(res => {
        this.banners = res ? res.map(val => val.photo) : ['http://you-one-stand.oss-cn-shanghai.aliyuncs.com/product-index/20190418160834.png'];
      }).catch(() => {
        this.banners = ['http://you-one-stand.oss-cn-shanghai.aliyuncs.com/product-index/20190418160834.png'];
      });
    },
    getcompany() {//获取公司信息
      let v = this;
      ...
      WXAJAX.POST({
        ...
        if (data) {
          ...
          data.forEach(function (i, k) {
            ...
            // if(i.isAssemble ) {
            //   i.price = i.assemblePrice/100;
            //   i.price = i.price.toFixed(2);
            //   // i.originPrice = i.assemblePrice/100;
            //   // i.originPrice = i.originPrice.toFixed(2);
            // }
            if(i.isKill) {
