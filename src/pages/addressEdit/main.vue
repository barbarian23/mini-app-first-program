  ...
  <div class="pt10">
    ...
    <div class="bgfff pl16 pt15 pb15 fs16 pr16">
      ...
      <div class="disflex jsbet">
        ...
        <input type="text"
               class="pt15 w100p phe8 flex1 over_1"
               readonly
               disabled
               placeholder="收货人地址"
               v-model="full_address">
        <span class="cblue pt15"
              @click="resetAddr">{{ lang.text_2 }}</span>

      </div>
      <input type="text"
             class="pt15 w100p phe8"
             placeholder="详细地址（楼层／门牌号)"
             v-model="district">
    ...
    <BottomButtonSmall :text="'保存'"
                       :url="'save'"
                       @btn_tap="btn_tap"
                       v-if="!editId"></BottomButtonSmall>
    <BottomButtonTwo :left="bottom.left"
                     :right="bottom.right"
                     @page_turn="page_turn"
                     v-else></BottomButtonTwo>
...
export default {
  name: "",
  components: { BottomButtonSmall, BottomButtonTwo, AuthorizeBox },
  data() {
    return {
      lang: LangUtils.getLangSrc().addressEdit,
      name: "",
      tel: "",
      full_address: "",
      district: "",
    ...
    getLocal(latitude, longitude) {
      ...
      v.myAmapFun.getRegeo({
        ...
        success: function(data) {
          ...
          v.full_address =
            v.locat.province + v.locat.city + v.locat.district + v.locat.addr;
        },
        fail: function(info) {
          v.locat.city = "失败";
          v.locat.addr = "重新定位";
