<template>
  <div>
    ...
    <div class="bg_line_blue pl16 pr15 cfff disflex jsbet h99 borderbox pt30">
      ...
      <img src="https://you-one-stand.oss-cn-shanghai.aliyuncs.com/yimai_photos/user/orderDetail.png" alt="" class="w57 h79" style="flex: 0 0 114upx;">
    </div>
    ...
    <div class="bgfff mt10">
      <div class="company disflex jsbet pl15 lh44 posre pr21" @click="toAppointMentPage">
        <div class="disflex align-cen h44">
          <img src="https://you-one-stand.oss-cn-shanghai.aliyuncs.com/yimai_photos/user/company_name.png" alt="" class="w16 h16 mr8">
          <span class="fs14 pr11">{{orderInfo.companyName}}</span>
          <img src="https://you-one-stand.oss-cn-shanghai.aliyuncs.com/yimai_photos/user/right.png" alt="" class="w10 h10">
        </div>
        <span v-if="orderInfo.state==1" class="corange fs14">{{ lang.text_0 }}</span>
      </div>
      <ProductCard
    ...
    <div class="bgfff lh44 fs16 c38 mb15 mt10">
      ...
      <div class="disflex jsbet pl15 pr16" v-if="orderInfo.state == 3 || orderInfo.state == 4">
        <span>{{ orderInfo.state == 3 ? lang.text_6 : lang.text_11 }}</span>
        <span class="ca8">{{orderInfo.state == 3 ? orderInfo.completionTime : orderInfo.cancellationTime}}</span>
      </div>
      <div class="disflex jsbet pl15 pr16">
        <span>{{ lang.text_7 }}</span>
        <span class="ca8">{{orderInfo.typeName}}</span>
      </div>

      <div class="pl15 pr16 " v-if="orderInfo.remark">
        <p>{{ lang.text_8 }}</p>
        <p class="fs14 ca8 lh18 pb10">
          {{orderInfo.remark}}
        </p>
      </div>
    ...
    <!--操作-->
    <div class="disflex row-reverse pt9 pb11 lh30 bgfff fs14 pr6 footer-btns bte8" v-if="orderInfo.state == 1 || orderInfo.state == 2">
      <span class="disinblock bgblue textc cfff bradius20 w90 mr10" @click="showTips(orderInfo, 'use')" v-if="orderInfo.state == 2">{{ lang.text_9 }}</span>
      <span class="disinblock textc be8 ca8 bradius20 w90 mr10" @click="showTips(orderInfo, 'cancel')">{{ lang.text_10 }}</span>
    </div>
...
<script>
  ...
  export default {
    ...
    data() {
      return{
        lang: LangUtils.getLangSrc().appointmentPack.orderDetail,
    ...
    methods: {
      inits(){
        ...
        return WXAJAX.POST({
        ...
        }, '', '/products/getAppointmentInfo').then((data) => {
          let price;
          ...
          data.appointmentTime = `${this.formatDate('yyyy-MM-dd hh:mm',data.startTime)} - ${this.formatDate('hh:mm', data.endTime)}`;
          data.typeName = data.serviceType == '1' ? '到店' : '上门';
          data.createTime = this.formatDate('yyyy-MM-dd hh:mm',data.createTime);
          data.cancellationTime = this.formatDate('yyyy-MM-dd hh:mm',data.cancellationTime);
          data.completionTime = this.formatDate('yyyy-MM-dd hh:mm',data.completionTime);
