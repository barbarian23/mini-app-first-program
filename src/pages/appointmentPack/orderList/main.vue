<template>
  <div>
    <div class="disflex jsaround pl16 pr16 lh49 ca8 fs16 ordertype bgfff">
      <span v-for="item in menus" :key="item.id" :class="order_type == item.id ? 'active' : '' " @click="order_type_tap(item.id)">{{item.label}}</span>
    </div>

    <!--订单-->
    <div class="pl15 pr15">
      <template v-if="cart_lists.length">
        <OrderItem v-for="(cart_item,k) in cart_lists"
                   :key="k"
                   :orderInfo="cart_item"
        ...
        <!--bottom-->
        <div class="textc lh42 fs12 ca8 bgf5f6" v-if="nodata">{{ lang.text_0 }}</div>
      </template>
      <div v-else>
        <NoData>{{ lang.text_1 }}</NoData>
      </div>
    </div>
    <div class="textc disflex w60 h60 bradius50p bgblue disinblock goToUserCenter" @click="page_turn('usercenter')">
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        lang: LangUtils.getLangSrc().appointmentPack.orderList,
        menus: [
          { id: 6, label: '未使用' },
          { id: 7, label: '已完成' },
        ],
    ...
    methods: {
      ...
      showDetail(info){
        wx.navigateTo({url: `/pages/appointmentPack/orderDetail/main?orderIds=${info.appointmentId}`})
      },
      ...
      inits() {//获取订单
        ...
        return WXAJAX.POST({
          state: v.order_type,
          pageNum: v.page,
        }, '', '/products/selectAppointmentPage').then((data) => {
          if (data) {
            let _datas = [];
