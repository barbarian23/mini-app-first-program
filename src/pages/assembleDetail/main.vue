<template>
  <div>
    <div class="commander-info textc pt15">
      <img :src="assembleInfo.avatarUrl"
           alt=""
    ...
    <div class="disflex btf5f6 pl15 pr15 pt15 mt15 mb15">
      <img :src="assembleInfo.goodsPhotoUrl"
           alt=""
           class="goods-img">
      <div class="disflex wrap"
           style="align-content: space-between;">
        <div class="title word-break-all w100p over_2 c38 fs14 lh25">
          {{assembleInfo.goodsName}}
        </div>
        <div class="w100p ca8 fs12">
          成团人数：{{assembleInfo.assembleNum}}人团
        </div>
        <div class="w100p corange">
          <span class="fs12">￥</span>
          <span class="fs20 fbold">{{(assemblePrice/100).toFixed(2)}}</span>
        </div>
      </div>
    </div>

    <div class="textc">
      <span class="fs14 c38">{{ lang.text_0 }}</span>
      <div class="mt14 count-down-cut-line">
        <CountDown :diffTime="cutTime"
                   className="bgfff pl10 pr10" />
      </div>
    </div>

    <div class="pt40 pl10 pr10">
      <div class="avatar-list disflex wrap jscen">
        <!-- <img :src="'https://you-one-stand.oss-cn-shanghai.aliyuncs.com/yimai_photos/user/card1_user.png'" alt="" class="w40 h40 bradius50p mr5 ml5 mb5"> -->
        <img :key="index"
    ...
    <div class="disflex fs18 textc lh49 fix_bottom bte8">
      <span class="w50p cfff bg-gradient-orange"
            @click="moreCollage">更多拼团
        <LoginIntercept class="w50p"></LoginIntercept>
      </span>
      <label class="w50p bg-gradient-red cfff"
             v-if="isShare">
        分享出去￥{{(assemblePrice/100).toFixed(2)}}
        <button style="display: none"
                open-type="share"></button>
        <LoginIntercept class="w50p"></LoginIntercept>
      </label>
      <span class="w50p cfff bg-gradient-red cfff"
            v-else-if="assembleInfo.state===1"
            @click="joinGroup">立即参团￥{{(assemblePrice/100).toFixed(2)}}
        <LoginIntercept class="w50p"></LoginIntercept>
      </span>
      <span class="w50p cfff bg-gradient-red cfff"
            v-else-if="assembleInfo.state!==1"
...
export default {
  ...
  data() {
    return {
      lang: LangUtils.getLangSrc().assembleDetail,
      // 订单信息
      orderInfo: {},
      //拼团id
      assembleId: '',
  ...
  methods: {
    //参团
    joinGroup() {
      wx.navigateTo({ url: '../prodDetail/main?goodId=' + this.assembleInfo.goodsId + "&join=true&assembleId=" + this.assembleId + '&cardId=' + (wx.getStorageSync('CARDID') || 0) + '&tel=' + (wx.getStorageSync('CARDTEL') || '') });
    },
    //我也要开团
    group() {
      wx.navigateTo({ url: '../prodDetail/main?goodId=' + this.assembleInfo.goodsId });
    },
    getInfo() {
      WXAJAX.POST({
        assembleId: this.assembleId
      }, '', '/assembles/getGoodsAssembleInfo').then(res => {
        this.assembleInfo = res;
        this.assemblePrice = res.goodSpecModelList[0].goodSpecModelList[0].assemblePrice
        ...
        let myArray = res.ordersModelList.find(item => item.userId === this.myUserId);
    ...
    dxAssembleMsg() {
      if (this.assembleInfo.assembleNum > this.assembleInfo.putAssemble) {
        this.msg = `还差${this.assembleInfo.assembleNum - this.assembleInfo.putAssemble}人，赶快邀请好友来拼团吧!`
      } else if (this.assembleInfo.assembleNum === this.assembleInfo.putAssemble) {
        this.msg = '恭喜您,拼团成功!';
      }
    },
    //更多拼团 回到商城页
  ...
  async onShareAppMessage(e) {
    ...
    addShareRecord(COMPANYID, 2, this.assembleId, uuid).then(res => { }, err => { });
    return {
