...
<script>
  ...
  export default {
    ...
    data(){
      return{
        lang: LangUtils.getLangSrc().distribution,
        totalMoney: '0.00',
        total: 0,
        page: 1,
        lists: [],
      ...
      getRoyaltyList(){
        new Promise((resolve, reject) => {
          ...
          WXAJAX.POST({
            type: 1,
            pageNum: this.page,
          }, '', '/record/selectRoyaltyRecord').then(res=>{
            wx.hideLoading();
            let list = JSON.parse(res.list) || [];
            this.lists = list.map(val=>{
              val.royaltyMoney = (val.royaltyMoney/100).toFixed(2);
              val.createTime = this.formatDate('yyyy-MM-dd hh:mm:ss', val.createTime);
...
<style>
  ...
  .total-money{
    height: 300upx;
    line-height: 1;
    background: url("https://you-one-stand.oss-cn-shanghai.aliyuncs.com/one-stand/photo/20190524/20190524112535.png") no-repeat;
    background-size: 100%;
    color: white;
    margin: 0 20upx;
    padding-left: 92upx;
