<template>
  <div>
    <div>
      <div class="disflex jsbet bgfff pl15 pr15 h49">
        <div class="fs16 fbold w100 textl h49 lh49">{{ lang.text_0 }}</div>
        <input type="text" v-model="videoItem.describes" class="pl15 pha8 fs14 lh49 h49 w100p c38 textr" placeholder="请输入视频名称">
      </div>
      <div class="disflex jsbet" v-if="videoItem.url == ''">
        <div class="bgfff textc h49 lh49 fs16 cblue btf5f6 bbf5f6 flex1" @click="chooseTypeShow = true">{{ lang.text_1 }}</div>
      </div>
      <div class="disflex jsbet" v-else>
        <div class="bgfff textc h49 lh49 fs16 cblue btf5f6 bbf5f6 flex1" @click="chooseTypeShow = true">{{ lang.text_2 }}</div>
        <div class="bgfff textc h49 lh49 fs16 cblue btf5f6 bbf5f6 flex1" @click="setCover">{{ lang.text_3 }}</div>
      </div>
      <div class="bgfff textc pt25 borderbox">
        <div v-if="videoItem.url != ''">
          <video :id="'myVideo'" class="bradius5 w100p h212 disblock" v-if="playVideo" :title="videoItem.describes" :src="videoItem.url" objectFit="cover" :poster="videoItem.cover" enable-danmu danmu-btn controls :autoplay="true">
          </video>
          <div v-else class="w100p h212 posre" @click="playVideo = true">
            <img :src="videoItem.cover || 'https://you-one-stand.oss-cn-shanghai.aliyuncs.com/one-www/photo/20190604/1559619324240.png'" mode="aspectFill" alt="" class="w100p h212">
            <img src="https://you-one-stand.oss-cn-shanghai.aliyuncs.com/one-www/photo/20190604/1559619365051.png" alt="" class="w50 h50 posab top0 left0 right0 bottom0 mauto">
          </div>
        </div>
        <img v-else src="https://you-one-stand.oss-cn-shanghai.aliyuncs.com/one-www/photo/20190522/1558493217500.png" alt="" mode="aspectFill" class="w100 h100">
        <div class="mt20 fs14 lh15 ca8 pb20">{{ lang.text_4 }}<br>请尽量上传10mb以内的视频</div>
      </div>
    </div>
    <div v-if="chooseTypeShow">
      <SelectorOne :title="'选择添加方式'" :status="chooseTypeShow" :allClass="uploadVideoTypes" :choose_id="uploadVideoType" @closeModal="chooseTypeShow=false" @choose_tap="choose">
      </SelectorOne>
    </div>
    <div v-show="dialog_show">
      <DialogBox :dialog_title="dialog_title" @btn_tap="dialog_tap" :dialog_ph="dialog_ph" :default="dialog_value" :left="'取消'" :right="'保存'"></DialogBox>
    </div>
    <BottomButtonSmall :text="'保存'" :url="'save'" @btn_tap="btn_tap"></BottomButtonSmall>
  </div>
</template>
<script>
import SelectorOne from '@/components/selectorOne'
import DialogBox from '@/components/dialogBox'
import WXAJAX from '@/utils/request'
import BottomButtonSmall from '@/components/bottom_button_small' //
import LangUtils from '../../utils/LangUtils';
export default {
  components: { DialogBox, SelectorOne, BottomButtonSmall },
  data() {
    return {
      lang: LangUtils.getLangSrc().editVideoExhibition,
      playVideo: false,
      type: '',
      dialog_show: false,
      dialog_title: '标题',
      dialog_ph: '请输入视频链接',
      dialog_value: '',
      chooseTypeShow: false,
      uploadVideoType: 0,
      uploadVideoTypes: [
        { name: '添加链接', id: 1 },
        { name: '本地视频', id: 2 }
      ],
      videoItem: {
        describes: '',
        url: '',
        cover: ''
      }
    }
  },
  onLoad() {
    let type = this.$root.$mp.query.type;
    wx.setNavigationBarTitle({
      title: type === 'edit' ? '编辑视频' : '新增视频'
    });
    if (type === 'edit') {
      let videoItem = wx.getStorageSync('editVideoExhibition')
      this.videoItem.describes = videoItem.videoTitle
      this.videoItem.url = videoItem.videoUrl
      this.videoItem.cover = videoItem.videoCover
      this.videoItem.videoId = videoItem.videoId
    }
  },
  onUnload() {
    wx.removeStorage({ key: 'editVideoExhibition' })
  },
  methods: {
    // 设置视频封面
    setCover() {
      wx.showLoading();
      wx.chooseImage({
        count: 1,
        sourceType: ['album', 'camera'],
        success: res => {
          WXAJAX.UploadImage(res.tempFilePaths[0])
            .then(data => {
              wx.hideLoading();
              data = JSON.parse(data);
              if (data.code == 200) {
                let cover = WXAJAX.imgBackUrl + data.data + '.primary.png';
                this.videoItem.cover = cover;
              }
            })
            .catch((err) => {
              wx.hideLoading();
              wx.showToast({
                title: '异常:'+JSON.stringify(err),
                duration: 2000,
                icon: 'none'
              })
            })
        },
        fail: err => {
          wx.hideLoading();
        }
      })
    },
    // 上传视频
    choose(id, name) {
      this.chooseTypeShow = false;
      if (id == 1) {
        this.dialog_show = true;
      }
      if (id == 2) {
        wx.chooseVideo({
          sourceType: ['camera', 'album'],
          compressed: false,
          camera: 'back',
          success: res => {
            wx.showLoading({
              title: '上传中…',
              mask: true
            });
            WXAJAX.UploadImage(res.tempFilePath)
              .then(data => {
                wx.hideLoading();
                data = JSON.parse(data);
                if (data.code == 200) {
                  let url = WXAJAX.imgBackUrl + data.data;
                  this.videoItem.url = url;
                }
              })
              .catch((err) => {
                console.log(err)
                wx.hideLoading();
                wx.showToast({
                  title: '异常:'+JSON.stringify(err),
                  duration: 2000,
                  icon: 'none'
                })
              })
          },
          fail: function(err) {
            wx.hideLoading();
            wx.showToast({
              title: '保存失败',
              duration: 2000,
              icon: 'none'
            })
          }
        })
      }
    },
    dialog_tap(str, dialog_value) {
      if (str == 'save') {
        this.videoItem.url = dialog_value
      }
      this.dialog_show = false;
    },
    btn_tap() { // 保存视频添加的视频
      if (!this.videoItem.describes.trim()) {
        wx.showToast({
          title: '请输入标题',
          duration: 2000,
          icon: 'none'
        });
        return;
      }
      if (!this.videoItem.url.trim()) {
        wx.showToast({
          title: '请添加视频',
          duration: 2000,
          icon: 'none'
        });
        return;
      }
      wx.showLoading();
      // 保存编辑
      if (this.$root.$mp.query.type == 'edit') {
        WXAJAX.POST({
          videoUrl: this.videoItem.url,
          videoCover: this.videoItem.cover,
          videoTitle: this.videoItem.describes,
          videoId: this.videoItem.videoId
        }, '', '/businessCardVideo/uodVideo').then(res => {
          wx.hideLoading();
          wx.showToast({
            title: '保存编辑视频成功',
            duration: 2000,
            icon: 'none'
          })
          let pagesArr = getCurrentPages()
          pagesArr[pagesArr.length - 1].onLoad()
          wx.navigateBack();
        }).catch(err => {
          wx.hideLoading();
          wx.showToast({
            title: '保存编辑视频失败',
            duration: 2000,
            icon: 'none'
          })
        })
      } else {
        WXAJAX.POST({
            videoUrl: this.videoItem.url,
            videoCover: this.videoItem.cover,
            videoTitle: this.videoItem.describes
          },
          '',
          '/businessCardVideo/addVideo'
        ).then((data) => {
          wx.hideLoading();
          wx.showToast({
            title: '添加视频成功！',
            duration: 1000,
            icon: 'none'
          });
          wx.navigateBack();
        }).catch((err) => {
          wx.hideLoading();
          wx.showToast({
            title: '添加视频失败！',
            duration: 2000,
            icon: 'none'
          });
        });
      }
    }
  }
}

</script>
<style>
page {
  background: #F5F5F6;
}

</style>
