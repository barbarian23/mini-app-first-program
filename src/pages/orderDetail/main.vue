  ...
  <div>
    ...
    <div class="bg_line_blue pl16 pr15 cfff disflex jsbet h99 borderbox pt30">
      <div>
        ...
        <p class="fs12 pt7" v-if="cart_lists[0].orderState !== 4">{{cart_lists[0].deliveryTimes}}</p>
        <p class="fs12 pt7" v-else>{{textInfo[orderState].subtext}}</p>
      </div>
      <img src="https://you-one-stand.oss-cn-shanghai.aliyuncs.com/yimai_photos/user/orderDetail.png" alt=""
           class="w57 h79">
    </div>
    ...
    <div class="bgfff lh44 fs16 c38 mb15 mt10">
      ...
      <div class="disflex jsbet pl15 pr16">
        ...
        <span class="ca8">{{cart_lists[0].payType == 1 ? lang.text_13 : '其他支付'}}</span>
      </div>
      <div class="disflex jsbet pl15 pr16">
      ...
      <div class="disflex row-reverse pt10 pb9 pr15" v-if="orderState == 2">
        <span class="textc be8 ca8 bradius20 w90 lh30 fs14" @click="dialog_show = !dialog_show">{{ lang.text_8 }}</span>
      </div>
    ...
    <div class="disflex row-reverse pt9 pb11 lh30 bgfff fs14">
      ...
      <!--待支付-->
      <span v-if="orderState == 1" @click="toPay(cart_lists[0].ordersId)" class="bgblue textc cfff bradius20 w110 mr15">{{ lang.text_9 }}</span>
      <span v-if="orderState == 1" @click="changeOrder('cancel',cart_lists[0].ordersId)"
            class="textc be8 ca8 bradius20 w110 mr10">{{ lang.text_10 }}</span>

      <!--待发货-->
      <span v-if="orderState == 2" @click="changeOrder('deliverGood',cart_lists[0].ordersId)"
            class="textc be8 ca8 bradius20 w90 mr15">{{ lang.text_11 }}</span>
      <!--待收货-->
      <span v-if="orderState == 3 " @click="changeOrder('getGood',cart_lists[0].ordersId,4)"
            class="bgblue textc cfff bradius20 w110 mr15">{{ lang.text_12 }}</span>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        lang: LangUtils.getLangSrc().orderDetail,
        dialog_show: false,
        refund_reason: '',//退款原因
        cart_lists: [
          {
    ...
    mounted() {
      ...
      let v = this;
      let arr = wx.getStorageSync('ordersId') || ''
    ...
    methods: {
      ...
      inits() {
        wx.showLoading();
        WXAJAX.POST({ordersIds: this.orderIds,}, '', '/orders/getPayOrderInfo').then(data => {
          if (data) {
            data.forEach((i, k) => {
              // this.default_addr = {
              //   name: i.receiveName,
            ...
            this.createTime = util.getdate(this.cart_lists[0].createTime, 'dateTime');
          } else {
            this.cart_lists = [];
          }
          wx.hideLoading();
      ...
      toPay(orderId) {//订单支付
        WXAJAX.ToPay({
          ordersId: orderId,
          orderinfoRequestList:this.cart_lists,
          addressId:this.default_addr.addressId
        }, '/orders/goTwoPay').then((data) => {
          this.inits();
        }).catch((err) => {
