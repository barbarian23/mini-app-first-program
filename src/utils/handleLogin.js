...
import { authSubscribeMessage } from './auth.js'
 ...
 * @param failedCallback 登录失败回调函数showToast
 *  @param userInfo getUserProfile直接登录时获取的data
 */
export function login(successCallback, failedCallback,userInfo) {
  wx.showLoading();
  wx.login({
    success(res) {
      if (res.code) {
        // 登录成功，获取用户信息
        getUserInfo(res.code, successCallback,userInfo);
...
export function redirectPage(url) {
  // if(url == '../distributionCenter/applyFor/main'){
 ...
 * @param data 用户信息
 */
export function getUserInfo(code, callback, data) {
  if(data){// 新版本登录
    postLogin(
      {
        code: code,
...
//   WXAJAX.GET({}, "/wx/subscribe-templates?accountType=USER", "/wechatApi").then(
//     res => {
//       if (res) {
//         uni.setStorageSync("templates", res);
//       }
//     },
//     err => {}
//   );
