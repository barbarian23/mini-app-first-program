...
//测试环境
//var baseUrl = 'http://test-api.haomp.top/api/userApi';
...
console.log(extJSON)
// 适配oem配置 禁止删除
var imgBackUrl = extJSON.ossUrl ? extJSON.ossUrl : "https://you-one-stand.oss-cn-shanghai.aliyuncs.com/";
var baseUrl = extJSON.baseUrl ? extJSON.baseUrl : "http://one-api.zhop.vn";//;////"https://api.youdomain.com"; // "http://192.168.0.188:443";
var companyKey = extJSON.companyKey ? extJSON.companyKey : "one-stand";
var uploadUrl = extJSON.uploadUrl ? extJSON.uploadUrl : "http://one-api.zhop.vn";


//小程序主体id 兼容 oem配置禁止删除
var smartMainId = extJSON.smartMainId ? extJSON.smartMainId : 67;
var mapKey = "YOURMAPKEY"
...
async function requestUrl(request, method, service) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (Object.keys(request).length) {
      ...
      // console.log("parame:", JSON.stringify(request))
      data = {
    ...
    wx.request({
      ...
      success: function (data) {
        ...
        let pages = wx.getStorageSync('pages')
        // 设置未登录可查询接口
        //personal/getDynamicInfoDetail
        //&& pages !== '/pages/prodDetail/main' 暂时不做跳转登陆的页面

        //console.log(service)
        if(!isLogin ){
          if(pages !== '/pages/index/main' &&  pages !== '/pages/dynamicDetail/main' && pages !== '/pages/commentDetail/main' ){
        ...
        * */
        if (
          ...
          if(data.data.message == '未到打卡时间'){
            resolve('beforeTheTime');
          }else{
            reject(data.data);
...
async function postLoginPromiseByTourists() {
  console.log("postLoginPromiseByTourists=========")
...
export function GET(request, successed, failed) {
  //优惠券合并
  //export function GET(request, url, path = "/userApi") {
  return new Promise((resolve, reject) => {
    wx.request({
      header: {
...
export function ToPay(request, service) {
  console.log("当前传值的datat", request)
  let data = request;
