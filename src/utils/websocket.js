...
let extJSON = wx.getExtConfigSync ? wx.getExtConfigSync() : {};
/**
 *
 * @param sendcode 登录成功时发送的code
 */
  export function connect(sendcode) {
  const url =  'ws://115.146.126.61:7443/';
  //const url = 'wss://api.youdomain.com/ws';
  //兼容oem配置 默认取url配置
...
export function jsMessage(res, sendcode) {
  ...
  if(messageData.code === 1010 || messageData.code === 1011){
    //console.log("接受消息 --- ",res.data);
  }
